...
class BranchingTree extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    drawBrach(canvas, Point(size.width / 2, size.height), branchLength,
        -Math.pi / 2, maxBranchSize);
  }
  ...
  void drawBrach(Canvas c, Point start, num length, num angle, double size) {
    if (size > 0) {
      ...
      Point end = Point(start.x + length * Math.cos(angle),
      ...
      for (int i = 0; i < subBranch; i++) {
        ...
        num newBranchAngle =
            angle + generator.nextDouble() * maxSubAngle - maxSubAngle / 2;
